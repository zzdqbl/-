...
•参赛队员及分工：高詹堯（技术架构调试）、冯哲（路径规划与避障参数调优）、童焯豪（传感器适配）、王永良（视觉二维码识别）、杜彧靓（系统部署）​

2. 硬件备案信息​

•主控平台：地平线 RDK X3​

•车体底盘：OriginCar 阿克曼前轮转向、后轮驱动底盘​

•感知设备：BMI088 六轴 IMU、USB 视觉相机、Aurora930 深度相机
...
│                    底盘驱动层                          │
│  ┌──────────┐  ┌──────────┐  ┌───────────────────┐  │
│  │ origincar │  │ IMU       │  │ 视觉/深度感知      │  │
│  │ _base     │  │ BMI088    │  │ USB/Aurora        │  │
│  └──────────┘  └──────────┘  └───────────────────┘  │
└─────────────────────────────────────────────────────┘
```
